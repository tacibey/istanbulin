document.addEventListener('DOMContentLoaded', () => {
    ...
    // --- YENİ BÖLÜM: SİTE PAYLAŞIM MANTIĞI ---
    function setupSiteShare() {
        ...
        const shareText = 'İstanbul İnteraktif Kültür Atlası "istanbulin" yayında. Teşrif etmez miydiniz? ⓘ istanbulin.org';
        const encodedShareText = encodeURIComponent(shareText);

        const shareLinks = {
            facebook: `https://www.facebook.com/sharer/sharer.php?u=${encodeURIComponent(shareUrl)}"e=${encodedShareText}`,
        ...
        sharePopup.addEventListener('click', (e) => {
            e.stopPropagation(); // Popup'ın içindeki tıklamalar popup'ı kapatmasın
        });
            ...
            option.addEventListener('click', () => {
                ...
                if (platform === 'copy') {
                    navigator.clipboard.writeText(shareText)
                        .then(() => showCopyNotification("Metin kopyalandı!"))
    ...
    function setupPWA() {
        ...
        navigator.serviceWorker.register('/sw.js').then(()=>console.log("Akıllı ServiceWorker başarıyla kaydedildi.")).catch(e=>console.log("ServiceWorker kaydı başarısız:",e));
    ...
    const lightTheme = L.tileLayer('https://server.arcgisonline.com/ArcGIS/rest/services/World_Topo_Map/MapServer/tile/{z}/{y}/{x}', { maxZoom: 19, maxNativeZoom: 19, noWrap: true });
    const darkTheme = L.tileLayer('https://services.arcgisonline.com/ArcGIS/rest/services/Canvas/World_Dark_Gray_Base/MapServer/tile/{z}/{y}/{x}', { maxZoom: 16, maxNativeZoom: 16, noWrap: true });
    ...
    function addMarkers(e) { const t=new Set(storage.get("lastSeenMarkers"));e.forEach(e=>{const o=!readMarkers.has(e.id.toString()),n=L.divIcon({className:o?"custom-marker-icon new-marker":"custom-marker-icon",html:"i",iconSize:[30,30],iconAnchor:[15,30],popupAnchor:[0,-25]}),i=L.marker([e.lat,e.lng],{icon:n});i.on("click",()=>{o&&(markAsRead(e.id),i.setIcon(L.divIcon({className:"custom-marker-icon",html:"i",iconSize:[30,30],iconAnchor:[15,30],popupAnchor:[0,-25]}))),window.location.hash=`/${e.id}`}),i.bindPopup(()=>createPopupContent(e),{minWidth:300}),markersLayer.addLayer(i),allMarkers[e.id]=i}),map.addLayer(markersLayer),openMarkerFromUrl();const o=e.map(e=>e.id);storage.set("lastSeenMarkers",o)}
    function openMarkerFromUrl() { const e=window.location.hash;if(e&&e.startsWith("#/")){goToMarker(e.substring(2))}}
    fetch('markers.json').then(e => e.json()).then(e => { allData = e; addMarkers(e); }).catch(e => { console.error("Veri çekilirken bir hata oluştu:", e); alert("Harita verileri yüklenirken bir sorun oluştu.") })
});
